...
function loader() {
	return '<div class="uk-text-center" style="margin-top:200px"><div class="md-preloader"><svg xmlns="http://www.w3.org/2000/svg" version="1.1" height="96" width="96" viewBox="0 0 75 75"><circle cx="37.5" cy="37.5" r="33.5" stroke-width="4"></circle></svg></div></div>';
}
...
$('body').on('click', '#delete-list-btn', function(e){
    e.preventDefault();
    
    var route = $(this).data('delete-item-route');
    var ids = [];
    
    $(".toggleable-btn-select.checked > i").each(function(){
        var id = $(this).attr("id").replace('toggle-item-', '');
        $.ajax({
    		method: 'DELETE',
